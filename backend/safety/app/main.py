...
async def _extraction_scheduler() -> None:
    ...
    from safety.logic import pattern_service
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="Adaptive Safety Intelligence — Elderly-Alone Engine",
        version="0.1.0",
        description=(
            "Independent twin of the pattern engine: learns an elderly person's "
            "routines, watches home-safety + health + security signals, and "
            "re-interprets every concern through a vulnerability lens to produce "
            "a deterministic safety assessment."
        ),
        lifespan=lifespan,
    )
